...
void init_snake(struct Snake* snake)
{
    ...
    for (i = 1; i < snake->length; i++) { /* 依次得到蛇身、蛇头的坐标 */
...
void move_snake(HANDLE hOut, struct Snake* snake)
{
    ...
    printf("  "); /* 清除蛇尾*/
    for (i = 1; i < snake->length; i++) { /* 后一节的坐标依次取代前一节的坐标 */
...
void get_food(HANDLE hOut, struct Snake* snake, struct Food* food)
{
    ...
    while (1) { /* 产生食物的条件：1.在游戏窗口内 2.不在蛇的身上 */
        ...
        for (i = 0; i < snake->length; i++) { /* 判断食物是否在蛇的身上，如果在蛇身上，则重新产生；否则，打印蛇身 */
...
{
    if (snake->x[snake->length - 1] == food->x && snake->y[snake->length - 1] == food->y) { /* 如果蛇头位置与食物位置相同，吃食物 */
        snake->length++; //吃一个食物，蛇身增长一节
        for (i = snake->length - 1; i >= 1; i--) { /* 蛇后节坐标依次赋值给蛇前一节的坐标，依次得到蛇身及蛇头的坐标 */
        ...
        snake->count++; //食物的个数增1
        if (snake->count % 5 == 0) { /* 当蛇吃Up_level个食物时，速度加快Up_speed毫秒并且升一级 */
            snake->speed -= 50;
        }
    }
}
...
void start_game()
{
    ...
    while (1) {
        ...
        if (if_die(snake) == 0 || ch == 27 || snake->speed == 50) { /* 游戏结束条件：1.蛇碰到自身 2.按Esc键 3.速度为50ms */
